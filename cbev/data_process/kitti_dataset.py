...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='train', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        assert mode in ['train', 'trainval', 'val', 'test'], 'Invalid mode: {}'.format(mode)
        self.mode = mode
    ...
    def __getitem__(self, index):
        if self.is_test:
            ...
            return self.load_img_with_val_targets(index)
        else:
    ...
    def load_img_only(self, index):
        ...
        t2 = time.time()
        print("Make Bev COST: {}".format(int(round(t2 * 1000))-int(round(t1 * 1000))))
    ...
    def load_img_with_val_targets(self, index):
        """Load images and targets for the training and validation phase"""
        sample_id = int(self.sample_id_list[index])
        img_path = os.path.join(self.image_dir, '{:06d}.png'.format(sample_id))
        lidarData = self.get_lidar(sample_id)
        calib = self.get_calib(sample_id)
        labels, has_labels = self.get_val_label(sample_id)
        if has_labels:
            labels[:, 1:8] = transformation.camera_to_lidar_box(labels[:, 1:8], calib.V2C, calib.R0, calib.P2)

        if self.lidar_aug:
            lidarData, labels[:, 1:8] = self.lidar_aug(lidarData, labels[:, 1:8])
    ...
    def get_val_label(self, idx):
        ...
        for line in open(label_path, 'r'):
            ...
            object_label = [cat_id, x, y, z, h, w, l, ry, \
                            float(line_parts[4]), float(line_parts[5]), float(line_parts[6]),\
                            float(line_parts[7]), truncated, occluded]
            labels.append(object_label)
    ...
    def build_val_targets(self, labels, hflipped):
        ...
        target = np.zeros((50,15))
        count = 0
        targets = {}

        for k in range(num_objects):
            #print(labels[k])
            cls_id, x, y, z, h, w, l, yaw = labels[k][:8]
            cls_id = int(cls_id)
            # Invert yaw angle
            yaw = -yaw
            if not ((minX <= x <= maxX) and (minY <= y <= maxY) and (minZ <= z <= maxZ)):
            ...
            center_y = (x - minX) / cnf.bound_size_x * hm_l * 4  # x --> y (invert to 2D image space)
            center_x = (y - minY) / cnf.bound_size_y * hm_w * 4  # y --> x
            center = np.array([center_x, center_y], dtype=np.float32)
            bbox_l = l / cnf.bound_size_x * hm_l * 4
            bbox_w = w / cnf.bound_size_y * hm_w * 4
            if hflipped:
                center[0] = hm_w - center[0] - 1
            #score centerx centery z h w l yaw class boxxmin, boxymin, boxxmax, boxymax truncted occulusion
            ob = np.array([1, center[0], center[1], z, h, bbox_w, bbox_l, yaw, cls_id,\
                           labels[k][8],labels[k][9],labels[k][10],labels[k][11],labels[k][12],labels[k][13]])
            target[k] = ob
            count += 1
